...
async function getPlayerWeaponsData(username) {
  ...
  const itemDocuments = await userWeaponsCollection
    .find(
...
async function getUserInventory(userId) {
  try {
    ...
    promises.push(
      await battlePassCollection
        .findOne(getUserIdPrefix(userId), {
          projection: {
            ss_passtier: 1,
            ss_coins: 1,
            ss_damage: 1,
    ...
    const inventory = {
      ...
      last_collected: userRow.inventory.daily_reward.last_collected_at,
      hat: userRow.equipped.hat,
      top: userRow.equipped.top,
      banner: userRow.equipped.banner,
      pose: userRow.equipped.pose,
      color: userRow.equipped.color,
      hat_color: userRow.equipped.hat_color,
      top_color: userRow.equipped.top_color,
      banner_color: userRow.equipped.banner_color,
      gadget: userRow.account.gadget,

      boxrarities: rarityPercentages,
      //  friends: userRow.social.friends || [],
      // requests: userRow.social.requests || [],
      serverlist,
      nearestRegion: getServerByCountry(
        userRow.account.country_code || "Unknown",
      ),
